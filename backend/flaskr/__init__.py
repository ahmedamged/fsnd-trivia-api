...
def create_app(test_config=None):
  ...
  def search_questions():
      ...
      try:
          questions_found = Question.query.order_by(Question.id).filter(Question.question.ilike('%{}%'.format(search)))
  ...
  def get_questions_by_category(category_id):
      ...
      category_questions = Question.query.order_by(Question.id).filter_by(category = category.id).all()
  ...
  def play():
      try:
          ...
          if (len(questions) > 0):
              random_questions = questions[random.randrange(0, len(questions))].format()
